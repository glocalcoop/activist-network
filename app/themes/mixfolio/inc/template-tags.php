...
function mixfolio_content_nav( $nav_id ) {
	...
	<?php if ( is_single() ) : // navigation links for single posts ?>
		...
		<?php if ( get_previous_post() ) { ?>
			<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'mixfolio' ) . '</span> %title' ); ?>
		<?php } else { ?>
		...
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'mixfolio' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) { ?>
			<div class="nav-previous">
				<?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'mixfolio' ) ); ?>
			</div>
		<?php } else { ?>
			<div class="nav-previous">
				&nbsp;
		...
		<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next">
				<?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'mixfolio' ) ); ?>
			</div>
		<?php endif; ?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<footer>
				...
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'mixfolio' ); ?></em>
					<br />
				<?php endif; ?>
				...
				<div class="comment-meta commentmetadata">
					...
					<?php
						...
						printf( __( '%1$s at %2$s', 'mixfolio' ), get_comment_date(), get_comment_time() ); ?>
					</time></a>
...
function mixfolio_posted_on() {
	if ( is_multi_author() && ! is_author() ) {
		printf( __( '<a class="posted-on"href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a>', 'mixfolio' ),
...
function mixfolio_posted_by() {
	if ( is_multi_author() && ! is_author() ) {
		printf( __( '<a class="author url fn n" href="%1$s" title="%2$s" rel="author">By %3$s</a>', 'mixfolio' ),
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( __( 'View all posts by %s', 'mixfolio' ), get_the_author_meta( 'display_name' ) ) ),
